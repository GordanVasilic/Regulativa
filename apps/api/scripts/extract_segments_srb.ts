...
// pdf-parse is CommonJS; use dynamic import in fallback to avoid ESM default issues
...
async function extractPageText(filePath: string): Promise<string[]> {
    try {
        ...
        try {
            ...
            res = await pdfParse(buf)
        } catch { }
        const text = normalizeText(String(res.text || ''))
        const parts = text.split(/\f+/).map((s) => s.trim()).filter((s) => s.length)
        return parts.length ? parts : [text]
    }
}

async function extractTextFromPdfData(u8: Uint8Array): Promise<string | null> {
    try {
        const loadingTask = getDocument({ data: u8 })
        const pdf = await loadingTask.promise
        const parts: string[] = []
        for (let i = 1; i <= pdf.numPages; i++) {
            ...
            const cleaned = text.replace(/\s+\n/g, '\n').replace(/\s{2,}/g, ' ').trim()
            parts.push(normalizeText(cleaned))
        }
        await pdf.cleanup()
        return parts.join('\n\n')
    } catch {
...
async function tryTextFromZipSource(url: string): Promise<string | null> {
    try {
        ...
        const pdfEntry = pick('.pdf')
        if (pdfEntry) {
            try {
                const buf = pdfEntry.getData()
                const u8 = new Uint8Array(buf.buffer, buf.byteOffset, buf.byteLength)
                const tx = await extractTextFromPdfData(u8)
                if (tx && tx.trim()) {
                    console.log(`[ZIP] PDF text length=${tx.length} sample="${tx.slice(0,120).replace(/\s+/g,' ')}"`)
                    return tx
                }
            } catch { }
        }
        ...
        if (docx) {
            try {
                ...
                const r = await mammoth.extractRawText({ buffer: docx.getData() })
                return normalizeText(String(r.value || ''))
            } catch { }
        }
        const rtf = pick('.rtf')
        if (rtf) {
            const raw = rtf.getData().toString('utf8')
...
async function main() {
    ...
    const LAW_TITLE_CONTAINS = process.env.LAW_TITLE_CONTAINS ? String(process.env.LAW_TITLE_CONTAINS) : null
    const JURISDICTION = String(process.env.JURISDICTION || 'SRB')

    let laws: { id: number; title: string; path_pdf: string; source_url?: string }[]
    if (ONE_LAW_ID) {
        laws = await all<{ id: number; title: string; path_pdf: string; source_url: string }>(
            'SELECT id, title, path_pdf, source_url FROM laws WHERE id = ? AND path_pdf IS NOT NULL',
            [ONE_LAW_ID]
        )
    } else if (LAW_TITLE_CONTAINS) {
        laws = await all<{ id: number; title: string; path_pdf: string; source_url: string }>(
            'SELECT id, title, path_pdf, source_url FROM laws WHERE jurisdiction = ? AND title LIKE ? AND path_pdf IS NOT NULL ORDER BY id DESC LIMIT 1',
            [JURISDICTION, `%${LAW_TITLE_CONTAINS}%`]
        )
    } else {
        laws = await all<{ id: number; title: string; path_pdf: string; source_url: string }>(
            'SELECT id, title, path_pdf, source_url FROM laws WHERE jurisdiction = ? AND path_pdf IS NOT NULL ORDER BY id ASC LIMIT ? OFFSET ?',
    ...
    for (const law of laws) {
        try {
            ...
            const PAGE_SEP = '\n\n'
            let fullText = pages.join(PAGE_SEP)
            if (!fullText.trim() && law['source_url'] && String(law['source_url']).toLowerCase().endsWith('.zip')) {
                const zipText = await tryTextFromZipSource(String(law['source_url']))
                if (zipText) fullText = zipText
            }
            ...
            const tolerantInline = (n: number) => new RegExp(`(?:^|${wsClass})${headingTokens}${wsClass}*${n}(?:${wsClass}+|${puncClass}|$)`, 'iu')
            ...
            if (maxNum > 0) {
                for (let n = 1; n <= maxNum; n++) {
                    ...
                    if (idx < 0) {
                        idx = fullText.search(tolerantInline(n))
                    }
                    if (idx >= 0) {
            ...
            if (maxNum > 0) {
                ...
                for (const n of missing) {
                    ...
                    let idx = fullText.search(tolerantInline(n))
                    if (idx < 0) idx = fullText.search(new RegExp(`(?:Č\\s*lan|C\\s*lan|Č\\s*l\\.|C\\s*l\\.|Ч\\s*лан|Ч\\s*л\\.)${wsClass}*${n}`, 'i'))
                    const page_hint = idx >= 0 ? pageForIndex(idx) : 1
                    const sliceStart = Math.max(0, idx >= 0 ? idx : 0)
                    const sliceEnd = Math.min(fullText.length, sliceStart + 2000)
                    const snippetRaw = fullText.slice(sliceStart, sliceEnd)
                    const text = normalizeText(snippetRaw.trim()) || `Član ${n}`
            ...
            console.log(`OK law_id=${law.id} title="${law.title}" pages=${pages.length} segments=${segments.length} source_url=${law['source_url'] || ''}`)
        } catch (e) {
            failed++
