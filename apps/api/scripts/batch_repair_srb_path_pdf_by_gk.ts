...
async function ensurePdfFromUrl(url: string, title: string, gazetteKey: string | null, pdfDir: string): Promise<string | null> {
  ...
  try {
    ...
    if (looksZip) {
      const tmpDir = path.join(process.cwd(), 'tmp', 'srb_zip_gk')
...
async function main() {
  const GK = process.env.GK ? String(process.env.GK) : '10_25'
  ...
  const items: { title: string; url: string; gazetteKey: string }[] = []
  ...
  const laws = await all<{ id: number; title: string; gazette_key: string | null }>(
    `SELECT id, title, gazette_key FROM laws WHERE jurisdiction='SRB' AND gazette_key = ? AND (path_pdf IS NULL OR path_pdf='') ORDER BY id ASC LIMIT ?`,
    [GK, LIMIT]
  )
  ...
  for (const law of laws) {
    ...
    let pickUrl = ''
    if (byNorm.has(keyTitle)) pickUrl = byNorm.get(keyTitle)![0].url
    if (!pickUrl) {
      for (const [k, arr] of byNorm.entries()) {
        if (k.includes(keyTitle.slice(0, Math.min(80, keyTitle.length)))) { pickUrl = arr[0].url; break }
      }
    }
    if (!pickUrl) continue
    const path_pdf = await ensurePdfFromUrl(pickUrl, law.title, law.gazette_key || GK, PDF_DIR)
    if (!path_pdf) continue
    ...
    await new Promise<void>((resolve) => {
      const env = { ...process.env, JURISDICTION: 'SRB', LAW_ID: String(law.id), DISABLE_HEURISTICS: '1' }
      const p = execFile(process.execPath, ['--import', 'tsx', 'scripts/extract_segments_rs.ts'], { cwd: ROOT, env })
      p.on('exit', () => resolve())
      p.on('error', () => resolve())
    })
    segmented.push(law.id)
  }
  console.log(JSON.stringify({ ok: true, updated_count: updated.length, segmented_count: segmented.length, updated_ids: updated }, null, 2))
  db.close()
}
